...
| **Document** | Architecture.md — v1.39 |
| **Date** | 11 August 2026 |
...
| Date | Version | Change |
|---|---|---|
| 11 Aug 2026 | 1.39 | **Adherence pie labelling — no Cancelled slice.** Dashboard and Reports status pies are adherence composition (Taken / Delayed / Missed only); `adherence()` / `checkInStatusBreakdown` / slice values unchanged. Title and caption make the exclusion explicit (cancelled count always; pending if non-zero). Closes the A-29 open on whether to add a Cancelled slice — ruled out. |
