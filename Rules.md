...
| **D14** | **Insert-only RLS and `RETURNING` are incompatible.** If a table's only policy is `FOR INSERT` with no `SELECT` policy, `INSERT … RETURNING` fails with Postgres error **42501**. Generate the id application-side (`crypto.randomUUID()`) and insert it explicitly. **Do not add a SELECT policy to make `RETURNING` work** — that would make the row set readable. Verified on `public.waitlist`, 17 August 2026. |
...
- [ ] **Never edit a workflow that contains a webhook trigger via the API.** `update_workflow` rotates the trigger node's `webhookId`. That changes the Meta callback URL and silently stops all inbound WhatsApp. **UI only** for webhook-bearing workflows (WF-2, WF-4a, WF-8). Sub-workflows without a webhook are safe to update programmatically.
