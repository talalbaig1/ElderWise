...
| **C15** | **Check-in status reaches a human only after DB→UI mapping, then a display formatter.** Never print a raw `checkins.status` enum (`responded`, `reminded`, `sent`, `scheduled`) to a doctor, PDF, CSV, or print view — those words are Track B vocabulary. Call `checkInStatusToUi(status, response_value)` (or load through a path that already maps), then `formatCheckInStatus` / `formatCheckInStatusWithResponse` when the response text must sit beside the label (e.g. `Answered no · some of them`). Do not rely on CSS `capitalize` for semantics. **`answered_no` is adherence-scored (denominator, no credit); `cancelled` is not.** Learned 11 August 2026 (A-29); extended 12 August 2026 (A1). |
